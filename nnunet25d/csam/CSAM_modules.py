"""
Official CSAM modules vendored from:
https://github.com/aL3x-O-o-Hung/CSAM

Local changes are limited to:
- keeping the code inside the nnunet25d package
- small safety guards around reduced dimensions
"""
...
class SemanticAttentionModule(nn.Module):
    def __init__(self, in_features, reduction_rate=16):
        ...
        hidden_features = max(in_features // reduction_rate, 1)
        self.linear = []
        self.linear.append(nn.Linear(in_features=in_features, out_features=hidden_features))
        self.linear.append(nn.ReLU())
        self.linear.append(nn.Linear(in_features=hidden_features, out_features=in_features))
