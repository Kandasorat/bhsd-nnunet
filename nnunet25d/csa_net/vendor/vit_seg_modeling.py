...
class NLBlockND_multicross(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels=None):
        ...
        if self.inter_channels is None:
            ...
            if self.inter_channels == 0:
                self.inter_channels = 1
        # Upstream created these layers inside forward() and hard-coded .cuda(),
        # so their parameters were never registered or optimized. Register the
        # same 16 heads once; this preserves the paper architecture and makes it
        # trainable, device-agnostic, and checkpointable.
        self.cross_attention = nn.ModuleList(
            [NLBlockND_multicross_block(in_channels=1024, inter_channels=64) for _ in range(16)]
        )
    def forward(self, x_thisBranch, x_otherBranch):
        outputs = [head(x_thisBranch, x_otherBranch) for head in self.cross_attention]
