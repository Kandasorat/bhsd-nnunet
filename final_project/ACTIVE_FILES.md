# Active Files

## Active code

- `../nnunet25d/baseline/`
- `../nnunet25d/common/`
- `../nnunet25d/csam/`
- `../nnunet25d/inference.py`
- `../nnunet25d/install_extension.py`

## Active configs

- `../configs/baseline_2d.yaml`
- `../configs/baseline_3d.yaml`
- `../configs/baseline_25d_3slide.yaml`
- `../configs/baseline_25d_3slide_binary.yaml`
- `../configs/baseline_25d_5slide_binary.yaml`
- `../configs/spacing_aware_25d.yaml`
- `../configs/csam_official_3slice.yaml`
- `../configs/csam_official_3slice_binary.yaml`
- `../configs/csam_official_volume32_fold0.yaml`
- `../configs/csa_net_official_3slice_fold0.yaml`

## Active scripts

- `../scripts/run_experiment.py`
- `../scripts/install_extension.py`
- `../scripts/verify_csam_official.py`
- `../scripts/verify_csam_official_binary.py`
- `../scripts/create_bhsd_binary_dataset.py`
- `../scripts/check_gadi_ready.py`

## Active docs

- `../README.md`
- `../docs/csam_official.md`
- `../docs/run_bhsd_binary_training.md`
- `../docs/run_25d_server.md`
- `../hpc/gadi/README.md`
- `../hpc/gadi/BASELINE_25D_FOLD0.md`
- `../hpc/gadi/ATTENTION_FOLD0.md`
- `../hpc/gadi/BINARY_BASELINES.md`
